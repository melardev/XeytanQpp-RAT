...
#include "../XeytaQpipi/XeytaQpipi/Logger.h"


QString wCharToString(const WCHAR* wstr, uint length) {
	return QString::fromUtf16(reinterpret_cast<const ushort *>(wstr), length);
}

QColor QStringToQColor(QString str)
{
	QColor color;
	...
	if (rgbList.size() != 3 && rgbList.size() != 4)
		goto error;
	bool isOk;
	color.setRed(rgbList[0].toInt(&isOk));
	if (!isOk) goto error;
	color.setGreen(rgbList[1].toInt(&isOk));
	if (!isOk) goto error;
	color.setBlue(rgbList[2].toInt(&isOk));
	if (!isOk) goto error;
	if (rgbList.size() == 4)
	{
		color.setAlpha(rgbList[3].toInt(&isOk));
		if (!isOk) goto error;
	}
	else
	{
	...
	return color; // If everything went ok
...
QString QColorToQString(QColor colour)
{
	QString s = QString::number(colour.red()) + "," +
		QString::number(colour.green()) + "," +
		QString::number(colour.blue());
	if (colour.alpha() != 255)
		s += "," + QString::number(colour.alpha());
	return s;
}
